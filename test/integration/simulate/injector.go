...
func (i *Injector) WipePrometheusData(ctx context.Context) error {
	return wipePrometheusData(ctx, i.prometheusNS, i.prometheusPods)
}
...
func (i *Injector) Inject(ctx context.Context, scenario *ScenarioBuilder) (*InjectionResult, error) {
	return i.injectWithOptions(ctx, scenario, true)
}
...
func (i *Injector) InjectFull(ctx context.Context, scenario *ScenarioBuilder) (*InjectionResult, error) {
	return i.injectWithOptions(ctx, scenario, false)
}

func (i *Injector) injectWithOptions(ctx context.Context, scenario *ScenarioBuilder, alertsOnly bool) (*InjectionResult, error) {
	...
	if err := runSimulate(ctx, i.projectRoot, scenarioFile, openmetricsFile, alertsOnly); err != nil {
	...
	if err := createTSDBBlocks(ctx, openmetricsFile, dataDir); err != nil {
		return nil, fmt.Errorf("failed to create TSDB blocks: %w", err)
	}

	// Step 4: Copy to Prometheus (with fallback to other pods)
	usedPod, err := copyBlocksToPrometheusWithFallback(ctx, dataDir, i.prometheusNS, i.prometheusPods)
	if err != nil {
