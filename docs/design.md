...
The results of the analyzer are provided through a set of Prometheus metrics. For the implementation details of how these metrics are produced, see [health-analysis-engine.md](health-analysis-engine.md).
...
lower the number, the more important the component is.
...
components the analyzer is aware of, even those that are not relevant for the cluster).
...
each node (both control plane and worker) could be modeled as an individual
component. However, this would require a reliable mechanism to extract the
...
health of the component, but does not require immediate action. This is
